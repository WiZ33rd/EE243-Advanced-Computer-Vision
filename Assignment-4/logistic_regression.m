...
lr = 1e-4;       % FILL IN AND MODIFY TO SEE CHANGE IN PERFORMANCE
...
theta = zeros(2048,101); % INITIALIZE; FILL IN
diff = 1;
epoch = 1;
...
while diff > 1e-10 && epoch < 4000
    ...
    % sigmoid function
    sigmoid = 1./(1+exp(-tefeature*theta));
    
    [~,pred] = max(sigmoid,[],2);
    pred = pred - 1;
    %calcaulte the right sample numbers for test accurancy
    Testright = sum(pred==telabel);
    test_accuracy = Testright/size(pred,1);
    ...
    lr = 0.0001 + 0.001*exp(-epoch/2000);
...
% CATEGORY PREDICTION
sigmoid = 1./(1+exp(-tefeature*theta));
[TPR, FPR] = getROC(sigmoid(:,50),telabel1hot(:,50));
